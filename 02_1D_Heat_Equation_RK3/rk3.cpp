...
int main(void) {
    ...
    // 初始化
    std::vector<double> x(nx + 1), un_cur(nx + 1), un_prev(nx + 1), un_tmp(nx + 1), u_e(nx + 1), error(nx + 1);
    ...
    for (long long k = 0; k < nt + 1; ++k) {
        ...
        for (long long i = 1; i < nx; ++i) {
            un_tmp[i] = 0.75 * un_prev[i] + 0.25 * un_cur[i] + 0.25 * beta * (un_cur[i + 1] - 2 * un_cur[i] + un_cur[i - 1]);
        }
        for (long long i = 1; i < nx; ++i) {
            un_cur[i] = 1.0 / 3.0 * un_prev[i] + 2.0 / 3.0 * un_tmp[i] + 2.0 / 3.0 * beta * (un_tmp[i + 1] - 2 * un_tmp[i] + un_tmp[i - 1]);
        }
        // 边界条件
        un_cur[0] = 0; un_cur[nx] = 0;
        un_prev = un_cur;
